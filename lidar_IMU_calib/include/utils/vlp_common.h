...
public:
  ...
  // 这里知识修改默认参数
  VelodyneCorrection(ModelType modelType = RS_16) : m_modelType(modelType) {
  ...
  void unpack_scan(const sensor_msgs::PointCloud2::ConstPtr &lidarMsg,
      ...
      outPointCloud.clear();
      outPointCloud.header = pcl_conversions::toPCL(lidarMsg->header);
      outPointCloud.height = 16;  // 这针对的是雷达线数
      outPointCloud.width = temp_pc.size()/16;
      outPointCloud.is_dense = false;
      outPointCloud.resize(outPointCloud.height * outPointCloud.width);
      ...
      for (int h = 0; h < outPointCloud.height; h++) {   // 两次遍历
        for (int w = 0; w < outPointCloud.width; w++) {
          TPoint point;
          licalib::PointXYZIT& rs_point=temp_pc[w+h*temp_pc.size()/16];
          point.x = rs_point.x;
          point.y = rs_point.y;
          point.z = rs_point.z;
...
private:
  void setParameters(ModelType modelType) {
    ...
    if (modelType == VLP_16) {
      ...
      // Set up cached values for sin and cos of all the possible headings
      for (uint16_t rot_index = 0; rot_index < ROTATION_MAX_UNITS; ++rot_index) {
        float rotation = angles::from_degrees(RS_ROTATION_RESOLUTION * rot_index);
        cos_rot_table_[rot_index] = cosf(rotation);
        sin_rot_table_[rot_index] = sinf(rotation);
      }
      ...
      float vert_correction[16] = { // 弧度制 参考P33
              -0.2617993877991494,
              -0.22689280275926285,
              -0.19198621771937624,
              -0.15707963267948966,
      ...
      for(unsigned int w = 0; w < 1800; w++) {
        for(unsigned int h = 0; h < 16; h++) {
          mRS16TimeBlock[w][h] = h * 2.8 * 1e-6 + w * 55.5 * 1e-6; /// VLP_16 16*1824
        }
      }
    }
  }
...
private:
  ...
  //RS_16的参数
  static const int RS_RAW_SCAN_SIZE = 3;
  static const int RS_SCANS_PER_BLOCK = 32;
  static const int RS_BLOCK_DATA_SIZE = (RS_SCANS_PER_BLOCK * RS_RAW_SCAN_SIZE);
  constexpr static const float RS_ROTATION_RESOLUTION = 0.01f;
  static const uint16_t RS_ROTATION_MAX_UNITS = 36000u;
  constexpr static const float RS_DISTANCE_RESOLUTION = 0.0025f;
